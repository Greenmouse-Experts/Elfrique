<template>
  <title>Form Payout Setting | Elfrique - Super Admin</title>
  <dash-header />
        ...
        <form @submit.prevent="updatePayout">
          <div class="row justify-content-center">
            <div class="col-lg-11 event-details-header">
              Update Form Payout Presetting
            </div>
            <!--Update Form Payout Presetting DIV-->
...
<script>
  ...
  export default {
    ...
    created() {
      PayoutService.getAllPayOut("Form").then((response) => {
    ...
    methods: {
      updatePayout() {
        ...
        PayoutService.updatePayOut("Form", this.payload).then((response) => {
